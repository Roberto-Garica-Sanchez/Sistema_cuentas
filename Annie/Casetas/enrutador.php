<?php 
    ...
    if(!empty($_POST['Operadores']) and $_POST['Operadores']=='Limpiar')    {
        if(file_exists(limpia.'/'.$elemento.'.php')){
            include_once(limpia.'/'.$elemento.'.php');
        }
        else if(file_exists(limpia.'/limpia_genericos.php')){
            include_once(limpia.'/limpia_genericos.php');
